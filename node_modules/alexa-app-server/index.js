...
var appServer = function(config) {
		...
		app_directories(app_dir).forEach(function(dir) {
			...
			try {
				...
				if (false!==config.debug) {
					...
					// set the main page for app
					self.express.get(endpoint + '/game', function(req, res) {
						app.game(req, res);
					})

					self.express.get(endpoint + '/media', function(req, res) {
						app.media(req, res);
					})
	...
	self.start = function() {
		...
		for (var key in self.apps) {
  		if (self.apps.hasOwnProperty(key)) {
    		views.push(path.join(app_dir, self.apps[key].dir + '/views'));
  		}
		}
		...
		var server = require('http').createServer(self.express);
		...
		// Add socket.io to apps
		for (var key in self.apps) {
  		if (self.apps.hasOwnProperty(key)) {
  			//console.log('### exports ' + JSON.stringify(self.apps[key].exports))
  			//console.log('## dir ' + self.apps[key].dir);
  			if (self.apps[key].exports.io) {
    			self.apps[key].exports.io(io);
    		}
  		}
		}
